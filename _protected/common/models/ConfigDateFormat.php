...
class ConfigDateFormat extends ActiveRecord {
    public function beforeSave($insert) {
        ...
        foreach ($this->getTableSchema()->columns as $column){ //Loop Column ทั้งหมด
            ...
            if(empty($attribute)){ //หากได้ค่าว่าง
                //ได้ตั้งค่า Alow null ใหม ถ้าไช่ให้ใส่ null
                $column->allowNull == 1 ? $this->setAttribute($column->name, NULL) : NULL;
            }else if ($column->dbType == 'datetime' && preg_match('/(\d{1,2}\s?\/\s?\d{1,2}\s?\/\s?\d{4}\s?\w*)/', $attribute)) {
                //ถ้ารูปแบบเป็น datetime
        ...
        foreach ($this->attributes as $column_name => $value) {
            if(preg_match('/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $value)){ //ถ้ามีค่าในรูปแบบ 2016-05-20 13:30:45
                if($value == '0000-00-00 00:00:00'){ //ถ้าไม่มีข้อมูล
                    ...
                    $result = $ymd[2] . '/' . $ymd[1] .'/' . $year . $date_time[1]; //ได้รูปแบบ วัน/เดือน/ปี ชั่วโมง:นาที:วินาที
